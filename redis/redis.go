...
func (client *Client) Randomkey() (string, error) {
	...
	return "", fmt.Errorf("redis randomkey value type:%v error", reflect.TypeOf(res).Name())
}
...
func writeToContainer(data [][]byte, val reflect.Value) error {
	...
	case reflect.Struct:
		for i := 0; i < len(data)/2; i = i + 1 {
			...
			if !field.IsValid() {
				name = strings.Title(string(data[i*2]))
				field = v.FieldByName(name)
				if !field.IsValid() {
					continue
				}
